...
// get_node_timeslot(const linkaddr_t *addr)
// {
//   if(addr != NULL && SQUATIX_ONE_SLOTFRAME_PERIOD > 0) {
//     return SQUATIX_LINKADDR_HASH(addr) % SQUATIX_ONE_SLOTFRAME_PERIOD;
//   } else {
//     return 0xffff;
//   }
// }
...
//  channel_offset = sf_handle;
  ...
  tsch_schedule_add_link(sf_node,
      ...
      0, 0); //get_node_timeslot(&linkaddr_node_addr)
}
...
/*---------------------------------------------------------------------------*/
// END OF DESIGN SHARED CELL FIRST
...
add_uc_link(const linkaddr_t *linkaddr)
{
  if(linkaddr != NULL) {
    ...
    tsch_schedule_add_link(sf_unicast,
        SQUATIX_UNICAST_SENDER_BASED ? LINK_OPTION_RX : LINK_OPTION_TX | UNICAST_SLOT_SHARED_FLAG,
...
init(uint16_t sf_handle)
{
  slotframe_handle = sf_handle;
  channel_offset = sf_handle;
  sf_eb = tsch_schedule_add_slotframe(slotframe_handle, SQUATIX_EBSF_PERIOD);
...
#if 0
/*---------------------------------------------------------------------------*/
struct SQUATIX_rule unicast_per_neighbor = {
  init,
  new_time_source,
  select_packet,
  child_added,
