...
void
uip_ds6_route_info_callback(uip_nd6_opt_route_info * rio, uip_ip6addr_t * next_hop) {
  PRINTF("UIP-RIO: callback\n");
  //TODO Preferences ?
  uip_ds6_route_t *found;
  if((found = uip_ds6_route_lookup(&rio->prefix)) == NULL && rio->rlifetime != 0) {
    //New route
    PRINTF("UIP-RIO: new route found !\n");
    PRINTF("UIP-RIO: type=%d\n", rio->type);
    PRINTF("UIP-RIO: flags=%d\n", rio->flagsreserved);
    PRINTF("UIP-RIO: length=%d\nUIP-RIO: Preflen=%d\nUIP-RIO: Prefix=",rio->len, rio->preflen);
    PRINT6ADDR(&rio->prefix);
    PRINTF("\nUIP-RIO: lifetime=%d\n", uip_ntohl(rio->rlifetime));
    uip_ds6_route_t *new_route;
    if((new_route = uip_ds6_route_add(&rio->prefix, rio->preflen, next_hop, UIP_DEFAULT_METRIC)) == NULL) {
