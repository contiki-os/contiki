...
void create_empty_nvm(void)
{
	...
	nvm_params->mode = CETIC_MODE_WSN_AUTOCONF | CETIC_MODE_WAIT_RA_MASK | CETIC_MODE_ROUTER_SEND_CONFIG |
...
int main(int argc, char * argv[])
{
	...
	if ( update_nvm_file || new_nvm_file ) {
		...
		if ( eth_addr_autoconf ) {
			mode_update( CETIC_MODE_ETH_AUTOCONF, boolconv( "eth-ip-autoconf", eth_addr_autoconf) );
		}
