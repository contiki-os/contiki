/* Current loop Source Code for CC2538 version microcontroller
 * Uses the collect method of the RIME stack for sending data TO the base
 * Uses the trickle method of the RIME stack for receiving data FROM the base
 * Uses ContikiMAC for radio duty cycling
 * Pulling PORTD.5 to GND will disable power saving
 */
...
void send_message(void* ptr) {
	...
	//Measure current loop sensor 1
	value = 0;
	for (i = 0; i < 100; i++) {
		value += adc_sensor.value(ADC_SENSOR_SENS1);
	}
	A1 = ((value / 100) * A0) / (2047 << 4);
	A1 = (A1 - 0.6) / 0.0238;

	//Measure current loop sensor 2
	value = 0;
	for (i = 0; i < 100; i++) {
		value += adc_sensor.value(ADC_SENSOR_SENS2);
	}
	A2 = ((value /100) * A0) / (2047 << 4);
	A2 = (A2 - 0.6) / 0.0238;
	
	//Measure soil moisture sensor 3
	value = 0;
	for (i = 0; i < 100; i++) {
		value += adc_sensor.value(ADC_SENSOR_SENS3);
	}
	A3 = ((value /100) * A0) / (2047 << 4);
	A3 = (A3 - 0.6) / 0.0238;
	
	//Measure soil moisture battery voltage
	value = 0;
	for (i = 0; i < 100; i++) {
		value += adc_sensor.value(ADC_SENSOR_SENS4);
	}
	Voltage = (((value /100) * A0) / (2047 << 4)) * 4.0325;
	...
	dec = A1;
	frac = A1 - dec;	
	sprintf(StringBuffer, "CL,A1=%d.%02u,", dec, (unsigned int) (frac*100));				//Add CL designator and Analog1 value to packet buffer
	dec = A2;
	frac = A2 - dec;	
...
static void process_line(void) {
	...
	else if (strncmp(usart_rx_buffer,"e",1) == 0)
	{
		TempAddr[2][0] = ConvertASCIICharToHex(usart_rx_buffer[1]) << 4;
		TempAddr[2][0] += ConvertASCIICharToHex(usart_rx_buffer[2]);
		TempAddr[2][1] = ConvertASCIICharToHex(usart_rx_buffer[3]) << 4;
		TempAddr[2][1] += ConvertASCIICharToHex(usart_rx_buffer[4]);
		TempAddr[2][2] = ConvertASCIICharToHex(usart_rx_buffer[5]) << 4;
		TempAddr[2][2] += ConvertASCIICharToHex(usart_rx_buffer[6]);
		TempAddr[2][3] = ConvertASCIICharToHex(usart_rx_buffer[7]) << 4;
		TempAddr[2][3] += ConvertASCIICharToHex(usart_rx_buffer[8]);
		TempAddr[2][4] = ConvertASCIICharToHex(usart_rx_buffer[9]) << 4;
		TempAddr[2][4] += ConvertASCIICharToHex(usart_rx_buffer[10]);
		TempAddr[2][5] = ConvertASCIICharToHex(usart_rx_buffer[11]) << 4;
		TempAddr[2][5] += ConvertASCIICharToHex(usart_rx_buffer[12]);
		TempAddr[2][6] = ConvertASCIICharToHex(usart_rx_buffer[13]) << 4;
		TempAddr[2][6] += ConvertASCIICharToHex(usart_rx_buffer[14]);
		TempAddr[2][7] = ConvertASCIICharToHex(usart_rx_buffer[15]) << 4;
		TempAddr[2][7] += ConvertASCIICharToHex(usart_rx_buffer[16]);	
	}
	else if (strncmp(usart_rx_buffer,"f",1) == 0)
	{
		TempAddr[3][0] = ConvertASCIICharToHex(usart_rx_buffer[1]) << 4;
		TempAddr[3][0] += ConvertASCIICharToHex(usart_rx_buffer[2]);
		TempAddr[3][1] = ConvertASCIICharToHex(usart_rx_buffer[3]) << 4;
		TempAddr[3][1] += ConvertASCIICharToHex(usart_rx_buffer[4]);
		TempAddr[3][2] = ConvertASCIICharToHex(usart_rx_buffer[5]) << 4;
		TempAddr[3][2] += ConvertASCIICharToHex(usart_rx_buffer[6]);
		TempAddr[3][3] = ConvertASCIICharToHex(usart_rx_buffer[7]) << 4;
		TempAddr[3][3] += ConvertASCIICharToHex(usart_rx_buffer[8]);
		TempAddr[3][4] = ConvertASCIICharToHex(usart_rx_buffer[9]) << 4;
		TempAddr[3][4] += ConvertASCIICharToHex(usart_rx_buffer[10]);
		TempAddr[3][5] = ConvertASCIICharToHex(usart_rx_buffer[11]) << 4;
		TempAddr[3][5] += ConvertASCIICharToHex(usart_rx_buffer[12]);
		TempAddr[3][6] = ConvertASCIICharToHex(usart_rx_buffer[13]) << 4;
		TempAddr[3][6] += ConvertASCIICharToHex(usart_rx_buffer[14]);
		TempAddr[3][7] = ConvertASCIICharToHex(usart_rx_buffer[15]) << 4;
		TempAddr[3][7] += ConvertASCIICharToHex(usart_rx_buffer[16]);
	}
	else if (strncmp(usart_rx_buffer,"g",1) == 0)
	{
		TempAddr[4][0] = ConvertASCIICharToHex(usart_rx_buffer[1]) << 4;
		TempAddr[4][0] += ConvertASCIICharToHex(usart_rx_buffer[2]);
		TempAddr[4][1] = ConvertASCIICharToHex(usart_rx_buffer[3]) << 4;
		TempAddr[4][1] += ConvertASCIICharToHex(usart_rx_buffer[4]);
		TempAddr[4][2] = ConvertASCIICharToHex(usart_rx_buffer[5]) << 4;
		TempAddr[4][2] += ConvertASCIICharToHex(usart_rx_buffer[6]);
		TempAddr[4][3] = ConvertASCIICharToHex(usart_rx_buffer[7]) << 4;
		TempAddr[4][3] += ConvertASCIICharToHex(usart_rx_buffer[8]);
		TempAddr[4][4] = ConvertASCIICharToHex(usart_rx_buffer[9]) << 4;
		TempAddr[4][4] += ConvertASCIICharToHex(usart_rx_buffer[10]);
		TempAddr[4][5] = ConvertASCIICharToHex(usart_rx_buffer[11]) << 4;
		TempAddr[4][5] += ConvertASCIICharToHex(usart_rx_buffer[12]);
		TempAddr[4][6] = ConvertASCIICharToHex(usart_rx_buffer[13]) << 4;
		TempAddr[4][6] += ConvertASCIICharToHex(usart_rx_buffer[14]);
		TempAddr[4][7] = ConvertASCIICharToHex(usart_rx_buffer[15]) << 4;
		TempAddr[4][7] += ConvertASCIICharToHex(usart_rx_buffer[16]);
	}
...
void ReadFromEEPROM(void) {
	...
	printf("@E=%02x%02x%02x%02x%02x%02x%02x%02x\r", TempAddr[2][0], TempAddr[2][1], TempAddr[2][2], TempAddr[2][3], TempAddr[2][4], TempAddr[2][5], TempAddr[2][6], TempAddr[2][7]);
	printf("@F=%02x%02x%02x%02x%02x%02x%02x%02x\r", TempAddr[3][0], TempAddr[3][1], TempAddr[3][2], TempAddr[3][3], TempAddr[3][4], TempAddr[3][5], TempAddr[3][6], TempAddr[3][7]);
	printf("@G=%02x%02x%02x%02x%02x%02x%02x%02x\r", TempAddr[4][0], TempAddr[4][1], TempAddr[4][2], TempAddr[4][3], TempAddr[4][4], TempAddr[4][5], TempAddr[4][6], TempAddr[4][7]);
...
PROCESS_THREAD(example_mesh_process, ev, data)
{
  ...
  while(1) {
	...
	if(etimer_expired(&periodic)) {					//The send message event timer expired variable
		...
		ctimer_set(&backoff_timer, (random_rand() % (2 * CLOCK_SECOND)), send_message, NULL);		//Wait 2 seconds for soil moisture node to settle and then send data
