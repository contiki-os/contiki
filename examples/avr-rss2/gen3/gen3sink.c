...
AUTOSTART_PROCESSES(&sinknode_process, &sleep_process, &batt_check_process);
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(sinknode_process, ev, data)
{
	...
	while(1) {
		...
		len += sprintf(&msg.buf[len]," V_MCU=%d.%02d SD.ERR=%d REPS=%d\n\r",p1,p2,err,lines);	
...
PROCESS_THREAD(batt_check_process, ev, data)
{
	...
	while(1) {
		...
		v_in=adc_read_v_in();
		if(v_in >= 2.80 && v_low==1){ //check if radio was stopped because of low power
			v_low=0;
			NETSTACK_RADIO.on();   
		}
	}
