...
/*---------------------------------------------------------------------------*/
PROCESS(udp_client_process, "UDP client bateria process");
...
//	bateria sensor
	...
	PRINTF("DATA send BATERIA: %i (%ld.%03d mV)",
			bateria,
			(long) mv,
...
//	uip_udp_packet_sendto(client_conn, buf, strlen(buf), &tmp_ipaddr,
//			UIP_HTONS(UDP_SERVER_PORT));
}
/*---------------------------------------------------------------------------*/
...
PROCESS_THREAD(udp_client_process, ev, data) {
	...
	PROCESS_BEGIN()
		...
		PROCESS_PAUSE();
		set_global_address();
		PRINTF("UDP client BATERIA process started\n");
