...
main(int argc, char **argv)
{
  ...
  printf("\n STARTING RABBITMQ\n");
  amqp_socket_t *socketRabbit = NULL;
  conn = amqp_new_connection();
  socketRabbit = amqp_tcp_socket_new(conn);
  amqp_socket_open(socketRabbit, HOST, atoi(PORT));
  amqp_login(conn, VHOST, 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, USERNAME, PASSWD);
  amqp_channel_open(conn, 1);
  amqp_get_rpc_reply(conn);

  {
	  propsrabbit._flags = AMQP_BASIC_CONTENT_TYPE_FLAG | AMQP_BASIC_DELIVERY_MODE_FLAG;
	  propsrabbit.content_type = amqp_cstring_bytes("text/plain");
	  propsrabbit.delivery_mode = 2; /* persistent delivery mode */
  }
  amqp_queue_declare(conn, 1, amqp_cstring_bytes(QUEUE), 0, 0, 0, 0,
			AMQP_EMPTY_TABLE);
